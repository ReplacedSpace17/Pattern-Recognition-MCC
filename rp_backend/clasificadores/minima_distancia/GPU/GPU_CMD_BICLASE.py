...
class CMD_BICLASE_GPU:
    ...
    def normalize(self, X):
        """
        Normalización Min-Max a los datos
        """
        X_min = cp.min(X, axis=0)
        X_max = cp.max(X, axis=0)
    ...
    def classify(self, data, selected_features, etiquetas, selected_class):
        df = pd.DataFrame(data)
        X = df[selected_features].values
        y = df[etiquetas[0]].values
        class_labels = np.unique(y)

        # Filtrar solo las dos clases seleccionadas
        X_filtered = X[np.isin(y, selected_class)]
        y_filtered = y[np.isin(y, selected_class)]

        # Normalización de los datos
        X_normalized = self.normalize(cp.asarray(X_filtered))

        results = {
            'K-Folds': self.k_fold_validation(X_normalized, y_filtered),
            'Stratified K-Folds': self.stratified_k_fold_validation(X_normalized, y_filtered),
            'Leave-One-Out': self.leave_one_out_validation(X_normalized, y_filtered)
        }

        cp.cuda.Device(0).synchronize()
        start_time = time.time()
        # Obtener las predicciones
        y_pred = self.minimum_distance_classifier(X_normalized, y_filtered, X_normalized)
        cp.cuda.Device(0).synchronize()
        end_time = time.time()
        ...
        # Matriz de Confusión
        conf_matrix = confusion_matrix(y_filtered, y_pred, labels=selected_class)
        conf_matrix_dict = {
            selected_class[i]: {selected_class[j]: int(conf_matrix[i, j]) for j in range(len(selected_class))}
            for i in range(len(selected_class))
        }

        final_results = {
            "K-Folds": results["K-Folds"],
            "Stratified K-Folds": results["Stratified K-Folds"],
            "Leave-One-Out": results["Leave-One-Out"],
            "Matriz de Confusión": conf_matrix_dict,
            "Precisión": precision_score(y_filtered, y_pred, average='weighted'),
            "Exactitud": accuracy_score(y_filtered, y_pred),
            "Sensibilidad (Recall)": recall_score(y_filtered, y_pred, average='weighted'),
            "Especificidad": f1_score(y_filtered, y_pred, average='weighted'),
            "Tipo de Distancia": self.distancia_type  # Agregar el tipo de distancia
        }

        # Estructura final para devolver el JSON
        final_json = {
            "type_classification": "biclase",
